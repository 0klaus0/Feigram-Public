...
const announcements = [
  ...
  },
  {
    ...
    body: [
      "修复重新登录后仍可能复用旧 Telegram session，导致 invalid auth key、msg_key mismatch 和连接超时的问题。",
      "同一个飞牛账户下只有一个 Telegram 账号时，重新登录会直接覆盖旧账号记录，不再生成新旧账号分叉。",
      "重置 Telegram client 时保留连接锁生命周期，避免重置过程中并发创建第二个 GramJS client。"
    ].join("\n")
  },
  {
    id: "release-2.0.74",
  ...
  },
  {
    ...
    body: [
      "修复 Go 下载调度器只修改任务副本，导致文件实际续传时界面仍显示旧重试文案的问题。",
      "任务开始或恢复下载后会立即清除旧错误和重试时间，并重新计算实时速度。",
      "缓存列表区分排队和等待续传；下载恢复后状态立即切换为下载中。",
      "跨 DC 连接波动仍会保留 .part 断点，并按队列策略自动续传。",
      "Go 下载服务版本更新为 0.13.1。"
    ].join("\n")
  },
  {
  ...
  },
  {
    ...
    body: [
      "修复管理员后台操作结果提示被网格布局拉伸、遮挡标题和切换页面后仍占据大块空间的问题。",
      "后台标题与标签保持固定，操作提示和页面内容进入统一可滚动区域。",
      "Go 账号连续两次健康检查通过后，会自动迁移该账号具备原生文件元数据的旧 HTTP 桥接任务。",
      "迁移任务保留已有 .part 文件和下载进度，以 native-mtproto 方式重新排队续传。",
      "正在运行、已完成和已取消的任务不会被强制改写。",
      "Go 下载服务版本更新为 0.9.6。"
    ].join("\n")
  },
  {
  ...
  },
  {
    ...
    body: [
      "健康检查优先复用已保存的媒体样本，首次检查会从任务中选择最小文件，避免与大文件下载争抢连接。",
      "健康检查超时现在明确显示发生在连接、授权、样本查找或文件分片读取阶段。",
      "Go 文件读取阶段和 Node 请求等待时间适当延长，降低慢网络下的误报。",
      "所有功能按钮新增统一按压和高亮反馈。",
      "扫码登录 Go、健康检查和验证码兜底会显示旋转图标、处理中状态和操作结果，并防止重复点击。",
      "Go 下载服务版本更新为 0.9.4。"
    ].join("\n")
  },
  {
  ...
  },
  {
    ...
    body: [
      "归档聊天改为独立目录显示，与原生 Telegram 一致。",
      "媒体查看器支持返回按钮、ESC 键关闭、背景点击关闭，图片/视频均可在应用内查看。",
      "消息缓存加速聊天切换，切换后立即显示缓存内容再后台刷新。",
      "未读标识即时清除：点击聊天后未读数字立即消失。",
      "实时新消息自动将该会话移到列表顶部并更新最新消息预览。",
      "修复非当前会话消息错误追加到当前消息列表的问题。",
      "修复新消息未读计数不递增的问题。"
    ].join("\n")
  },
  {
